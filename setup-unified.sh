...
# Usage: ./setup-unified.sh <environment> [branch]
# Example: ./setup-unified.sh dev R1
# Example: ./setup-unified.sh prod main
...
if [ "$ENVIRONMENT" == "prod" ]; then
    DOMAIN="ai.epic.dm"
    APP_DIR="/opt/livekit1"
    DB_NAME="epic_voice_db"
    DB_USER="epic_user"
elif [ "$ENVIRONMENT" == "dev" ]; then
    DOMAIN="dev.ai.epic.dm"
    APP_DIR="/opt/livekit1-dev"
    DB_NAME="epic_voice_dev"
    DB_USER="epic_dev_user"
else
    echo "Invalid environment. Use 'dev' or 'prod'"
    exit 1
fi

# Use environment variable if set, else use default
DB_PASS="${DB_PASSWORD:-secure_password_here}"  # CHANGE THIS OR SET DB_PASSWORD ENV VAR
...
echo "========================================"
cp $APP_DIR/epic-voice-app/.env.example $APP_DIR/epic-voice-app/.env.local
...
<VirtualHost *:80>
    ServerName $DOMAIN
    DocumentRoot $APP_DIR/epic-voice-app/.next
...
[Service]
User=$USER
WorkingDirectory=$APP_DIR/epic-voice-app
