...
export async function listAllUsers() {
  ...
  return users.map(user => ({
    ...
    subscription: user.ownedOrgs[0]?.subscription ? {
      status: user.ownedOrgs[0].subscription.status,
      trialEndsAt: user.ownedOrgs[0].subscription.trialEndsAt,
      currentPeriodEnd: user.ownedOrgs[0].subscription.currentPeriodEnd
    } : null
  }))
}
