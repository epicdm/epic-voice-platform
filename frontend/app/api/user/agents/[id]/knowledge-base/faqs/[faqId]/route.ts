...
export async function PUT(
  ...
  try {
    ...
    if (isLocalhost) {
      ...
      console.log('🔓 LOCALHOST: Using test user for FAQ update');
    } else {
    ...
    const response = await fetch(`${BACKEND_URL}/api/user/agents/${agentId}/knowledge-base/faqs/${faqId}`, {
      method: 'PUT',
      headers: {
    ...
    console.error('Knowledge Base FAQ Update API Error:', error)
    return NextResponse.json(
      {
        success: false,
        error: {
...
export async function DELETE(
  ...
  try {
    ...
    if (isLocalhost) {
      userEmail = 'giraud.eric@gmail.com';
      console.log('🔓 LOCALHOST: Using test user for FAQ delete');
    } else {
      const session = await auth();
    ...
    console.error('Knowledge Base FAQ Delete API Error:', error)
    return NextResponse.json(
      {
        success: false,
        error: {
